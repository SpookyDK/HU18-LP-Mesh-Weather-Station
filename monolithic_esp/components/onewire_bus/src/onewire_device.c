...
esp_err_t onewire_new_device_iter(onewire_bus_handle_t bus,
    ...
    ESP_RETURN_ON_FALSE(bus && ret_iter, ESP_ERR_INVALID_ARG, TAG,
                        "invalid argument");

    onewire_device_iter_t *iter = calloc(1, sizeof(onewire_device_iter_t));
    ESP_RETURN_ON_FALSE(iter, ESP_ERR_NO_MEM, TAG,
                        "no mem for device iterator");
...
esp_err_t onewire_device_iter_get_next(onewire_device_iter_handle_t iter,
    ...
    ESP_RETURN_ON_FALSE(iter && dev, ESP_ERR_INVALID_ARG, TAG,
                        "invalid argument");
    // we don't treat iterator ending and ESP_ERR_NOT_FOUND as an error
    // condition, so just print debug message here
    ...
    ESP_RETURN_ON_ERROR(
        onewire_bus_write_bytes(bus, (uint8_t[]){ONEWIRE_CMD_SEARCH_NORMAL}, 1),
        TAG, "send ONEWIRE_CMD_SEARCH_NORMAL failed");

    uint8_t last_zero = 0;
    for (uint16_t rom_bit_index = 0;
         rom_bit_index < sizeof(onewire_device_address_t) * 8;
         rom_bit_index++) {
        uint8_t rom_byte_index = rom_bit_index / 8;
        uint8_t rom_bit_mask =
            1 << (rom_bit_index % 8); // calculate byte index and bit mask in
                                      // advance for convenience

        uint8_t rom_bit = 0;
        uint8_t rom_bit_complement = 0;
        ESP_RETURN_ON_ERROR(
            onewire_bus_read_bit(bus, &rom_bit), TAG,
            "read rom_bit error"); // write 1 bit to read from the bus
        ESP_RETURN_ON_ERROR(
            onewire_bus_read_bit(bus, &rom_bit_complement), TAG,
            "read rom_bit_complement error"); // read a bit and its complement
        ...
        if (rom_bit !=
            rom_bit_complement) { // There are only 0s or 1s in the bit of the
                                  // participating ROM numbers.
            search_direction = rom_bit; // just go ahead
        } else { // There are both 0s and 1s in the current bit position of the
                 // participating ROM numbers. This is a discrepancy.
            if (rom_bit_index <
                iter->last_discrepancy) { // current id bit is before the last
                                          // discrepancy bit
                search_direction =
                    (iter->rom_number[rom_byte_index] & rom_bit_mask)
                        ? 0x01
                        : 0x00; // follow previous way
            } else {
                search_direction = (rom_bit_index == iter->last_discrepancy)
                                       ? 0x01
                                       : 0x00; // search for 0 bit first
            }
        ...
        if (search_direction ==
            1) { // set corrsponding rom bit by search direction
            iter->rom_number[rom_byte_index] |= rom_bit_mask;
        } else {
            iter->rom_number[rom_byte_index] &= ~rom_bit_mask;
        }
        ...
        ESP_RETURN_ON_ERROR(onewire_bus_write_bit(bus, search_direction), TAG,
                            "write direction bit error");
    }
    ...
    // check crc
    ESP_RETURN_ON_FALSE(onewire_crc8(0, iter->rom_number, 7) ==
                            iter->rom_number[7],
                        ESP_ERR_INVALID_CRC, TAG, "bad device crc");
