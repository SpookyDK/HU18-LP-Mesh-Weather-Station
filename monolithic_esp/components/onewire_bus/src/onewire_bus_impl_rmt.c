...
#define ONEWIRE_RMT_RESOLUTION_HZ                                              \
    1000000 // RMT channel default resolution for 1-wire bus, 1MHz, 1tick = 1us
...
// for chips whose RMT RX channel doesn't support ping-pong, we need the user to
// tell the maximum number of bytes will be received
#if CONFIG_IDF_TARGET_ESP32 || CONFIG_IDF_TARGET_ESP32S2
...
#define ONEWIRE_RESET_WAIT_DURATION                                            \
    200 // how long should master wait for device to show its presence
#define ONEWIRE_RESET_PRESENCE_WAIT_DURATION_MIN                               \
    15 // minimum duration for master to wait device to show its presence
#define ONEWIRE_RESET_PRESENCE_DURATION_MIN                                    \
    60 // minimum duration for master to recognize device as present
...
// refer to
// https://www.maximintegrated.com/en/design/technical-documents/app-notes/3/3829.html
// for more information
#define ONEWIRE_SLOT_RECOVERY_DURATION                                         \
    5 // recovery time between each bit, should be longer in parasite power mode
#define ONEWIRE_SLOT_BIT_SAMPLE_TIME                                           \
    15 // how long after bit start pulse should the master sample from the bus
...
typedef struct {
    ...
    rmt_encoder_handle_t
        tx_bytes_encoder; /*!< used to encode commands and data */
    rmt_encoder_handle_t
        tx_copy_encoder; /*!< used to encode reset pulse and bits */

    rmt_symbol_word_t *rx_symbols_buf; /*!< hold rmt raw symbols */

    size_t
        max_rx_bytes; /*!< buffer size in byte for single receive transaction */
...
const static rmt_receive_config_t onewire_rmt_rx_config = {
    ...
    .signal_range_max_ns =
        (ONEWIRE_RESET_PULSE_DURATION + ONEWIRE_RESET_WAIT_DURATION) * 1000,
};

static esp_err_t onewire_bus_rmt_read_bit(onewire_bus_handle_t bus,
                                          uint8_t *rx_bit);
static esp_err_t onewire_bus_rmt_write_bit(onewire_bus_handle_t bus,
                                           uint8_t tx_bit);
static esp_err_t onewire_bus_rmt_read_bytes(onewire_bus_handle_t bus,
                                            uint8_t *rx_buf,
                                            size_t rx_buf_size);
static esp_err_t onewire_bus_rmt_write_bytes(onewire_bus_handle_t bus,
...
IRAM_ATTR
bool onewire_rmt_rx_done_callback(rmt_channel_handle_t channel,
...
------------+       +------+          +-----------
            ...
            1       2      3          4
              ...
              [0].0  [0].1     [1].0    [1].1
*/
static bool onewire_rmt_check_presence_pulse(rmt_symbol_word_t *rmt_symbols,
                                             size_t symbol_num) {
    bool ret = false;
    if (symbol_num >= 2) { // there should be at lease 2 symbols(3 or 4 edges)
        if (rmt_symbols[0].level1 == 1) { // bus is high before reset pulse
            if (rmt_symbols[0].duration1 >
                    ONEWIRE_RESET_PRESENCE_WAIT_DURATION_MIN &&
                rmt_symbols[1].duration0 >
                    ONEWIRE_RESET_PRESENCE_DURATION_MIN) {
                ret = true;
            }
        } else { // bus is low before reset pulse(first pulse after rmt channel
                 // init)
            if (rmt_symbols[0].duration0 >
                    ONEWIRE_RESET_PRESENCE_WAIT_DURATION_MIN &&
                rmt_symbols[1].duration1 >
                    ONEWIRE_RESET_PRESENCE_DURATION_MIN) {
                ret = true;
            }
        }
...
static void onewire_rmt_decode_data(rmt_symbol_word_t *rmt_symbols,
                                    size_t symbol_num, uint8_t *rx_buf,
                                    size_t rx_buf_size) {
    size_t byte_pos = 0;
    size_t bit_pos = 0;
...
esp_err_t onewire_new_bus_rmt(const onewire_bus_config_t *bus_config,
    ...
    ESP_RETURN_ON_FALSE(bus_config && rmt_config && ret_bus,
                        ESP_ERR_INVALID_ARG, TAG, "invalid argument");

    bus_rmt = calloc(1, sizeof(onewire_bus_rmt_obj_t));
    ESP_RETURN_ON_FALSE(bus_rmt, ESP_ERR_NO_MEM, TAG,
                        "no mem for onewire_bus_rmt_obj_t");
    ...
    ESP_GOTO_ON_ERROR(
        rmt_new_rx_channel(&onewire_rx_channel_cfg, &bus_rmt->rx_channel), err,
        TAG, "create rmt rx channel failed");
    ...
    ESP_GOTO_ON_ERROR(
        rmt_new_tx_channel(&onewire_tx_channel_cfg, &bus_rmt->tx_channel), err,
    ...
    bus_rmt->rx_symbols_buf =
        malloc(rmt_config->max_rx_bytes * sizeof(rmt_symbol_word_t) * 8);
    ESP_GOTO_ON_FALSE(bus_rmt->rx_symbols_buf, ESP_ERR_NO_MEM, err, TAG,
                      "no mem to store received RMT symbols");
    bus_rmt->max_rx_bytes = rmt_config->max_rx_bytes;

    bus_rmt->receive_queue = xQueueCreate(1, sizeof(rmt_rx_done_event_data_t));
    ESP_GOTO_ON_FALSE(bus_rmt->receive_queue, ESP_ERR_NO_MEM, err, TAG,
                      "receive queue creation failed");

    bus_rmt->bus_mutex = xSemaphoreCreateMutex();
    ESP_GOTO_ON_FALSE(bus_rmt->bus_mutex, ESP_ERR_NO_MEM, err, TAG,
                      "bus mutex creation failed");

    // register rmt rx done callback
    rmt_rx_event_callbacks_t cbs = {.on_recv_done =
                                        onewire_rmt_rx_done_callback};
    ESP_GOTO_ON_ERROR(
        rmt_rx_register_event_callbacks(bus_rmt->rx_channel, &cbs, bus_rmt),
        err, TAG, "enable rmt rx channel failed");

    // enable rmt channels
    ESP_GOTO_ON_ERROR(rmt_enable(bus_rmt->rx_channel), err, TAG,
                      "enable rmt rx channel failed");
    ESP_GOTO_ON_ERROR(rmt_enable(bus_rmt->tx_channel), err, TAG,
                      "enable rmt tx channel failed");
    ...
    ESP_GOTO_ON_ERROR(rmt_transmit(bus_rmt->tx_channel,
                                   bus_rmt->tx_copy_encoder, &release_symbol,
                                   sizeof(release_symbol),
                                   &onewire_rmt_tx_config),
                      err, TAG, "release bus failed");
...
static esp_err_t onewire_bus_rmt_del(onewire_bus_handle_t bus) {
    onewire_bus_rmt_obj_t *bus_rmt =
        __containerof(bus, onewire_bus_rmt_obj_t, base);
...
static esp_err_t onewire_bus_rmt_reset(onewire_bus_handle_t bus) {
    ...
    ESP_GOTO_ON_ERROR(rmt_receive(bus_rmt->rx_channel, bus_rmt->rx_symbols_buf,
                                  sizeof(rmt_symbol_word_t) * 2,
                                  &onewire_rmt_rx_config),
                      err, TAG, "1-wire reset pulse receive failed");
    ESP_GOTO_ON_ERROR(rmt_transmit(bus_rmt->tx_channel,
                                   bus_rmt->tx_copy_encoder,
                                   &onewire_reset_pulse_symbol,
                                   sizeof(onewire_reset_pulse_symbol),
                                   &onewire_rmt_tx_config),
                      err, TAG, "1-wire reset pulse transmit failed");

    // wait and check presence pulse
    rmt_rx_done_event_data_t rmt_rx_evt_data;
    ESP_GOTO_ON_FALSE(xQueueReceive(bus_rmt->receive_queue, &rmt_rx_evt_data,
                                    pdMS_TO_TICKS(1000)) == pdPASS,
                      ESP_ERR_TIMEOUT, err, TAG,
                      "1-wire reset pulse receive timeout");
    if (onewire_rmt_check_presence_pulse(rmt_rx_evt_data.received_symbols,
                                         rmt_rx_evt_data.num_symbols) ==
        false) {
...
static esp_err_t onewire_bus_rmt_write_bytes(onewire_bus_handle_t bus,
    ...
    ESP_GOTO_ON_ERROR(rmt_transmit(bus_rmt->tx_channel,
                                   bus_rmt->tx_bytes_encoder, tx_data,
                                   tx_data_size, &onewire_rmt_tx_config),
                      err, TAG, "1-wire data transmit failed");
    // wait the transmission to complete
    ESP_GOTO_ON_ERROR(rmt_tx_wait_all_done(bus_rmt->tx_channel, 50), err, TAG,
                      "wait for 1-wire data transmit failed");
...
// While receiving data, we use rmt transmit channel to send 0xFF to generate
// read pulse, at the same time, receive channel is used to record weather the
// bus is pulled down by device.
static esp_err_t onewire_bus_rmt_read_bytes(onewire_bus_handle_t bus,
                                            uint8_t *rx_buf,
                                            size_t rx_buf_size) {
    onewire_bus_rmt_obj_t *bus_rmt =
        __containerof(bus, onewire_bus_rmt_obj_t, base);
    esp_err_t ret = ESP_OK;
    ESP_RETURN_ON_FALSE(rx_buf_size <= bus_rmt->max_rx_bytes,
                        ESP_ERR_INVALID_ARG, TAG,
                        "rx_buf_size too large for buffer to hold");
    ...
    ESP_GOTO_ON_ERROR(rmt_receive(bus_rmt->rx_channel, bus_rmt->rx_symbols_buf,
                                  rx_buf_size * 8 * sizeof(rmt_symbol_word_t),
                                  &onewire_rmt_rx_config),
                      err, TAG, "1-wire data receive failed");
    ESP_GOTO_ON_ERROR(rmt_transmit(bus_rmt->tx_channel,
                                   bus_rmt->tx_bytes_encoder, tx_buffer,
                                   sizeof(tx_buffer), &onewire_rmt_tx_config),
                      err, TAG, "1-wire data transmit failed");
    ...
    ESP_GOTO_ON_FALSE(xQueueReceive(bus_rmt->receive_queue, &rmt_rx_evt_data,
                                    pdMS_TO_TICKS(1000)) == pdPASS,
                      ESP_ERR_TIMEOUT, err, TAG, "1-wire data receive timeout");
    onewire_rmt_decode_data(rmt_rx_evt_data.received_symbols,
                            rmt_rx_evt_data.num_symbols, rx_buf, rx_buf_size);
...
static esp_err_t onewire_bus_rmt_write_bit(onewire_bus_handle_t bus,
    ...
    onewire_bus_rmt_obj_t *bus_rmt =
        __containerof(bus, onewire_bus_rmt_obj_t, base);
    const rmt_symbol_word_t *symbol_to_transmit =
        tx_bit ? &onewire_bit1_symbol : &onewire_bit0_symbol;
    esp_err_t ret = ESP_OK;
    ...
    // transmit bit
    ESP_GOTO_ON_ERROR(rmt_transmit(bus_rmt->tx_channel,
                                   bus_rmt->tx_copy_encoder, symbol_to_transmit,
                                   sizeof(rmt_symbol_word_t),
                                   &onewire_rmt_tx_config),
                      err, TAG, "1-wire bit transmit failed");
    // wait the transmission to complete
    ESP_GOTO_ON_ERROR(rmt_tx_wait_all_done(bus_rmt->tx_channel, 50), err, TAG,
                      "wait for 1-wire bit transmit failed");
...
static esp_err_t onewire_bus_rmt_read_bit(onewire_bus_handle_t bus,
    ...
    ESP_GOTO_ON_ERROR(
        rmt_transmit(bus_rmt->tx_channel, bus_rmt->tx_copy_encoder,
                     &onewire_bit1_symbol, sizeof(rmt_symbol_word_t),
                     &onewire_rmt_tx_config),
        err, TAG, "1-wire bit transmit failed");
    ...
    ESP_GOTO_ON_FALSE(xQueueReceive(bus_rmt->receive_queue, &rmt_rx_evt_data,
                                    pdMS_TO_TICKS(1000)) == pdPASS,
                      ESP_ERR_TIMEOUT, err, TAG, "1-wire bit receive timeout");
    uint8_t rx_buffer = 0;
    onewire_rmt_decode_data(rmt_rx_evt_data.received_symbols,
                            rmt_rx_evt_data.num_symbols, &rx_buffer,
                            sizeof(rx_buffer));
