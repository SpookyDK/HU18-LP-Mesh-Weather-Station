...
#ifndef CONFIG_BMX280_TIMEOUT
#define CONFIG_BMX280_TIMEOUT 1000 / portTICK_PERIOD_MS
#endif
...
static esp_err_t bmx280_probe_address(bmx280_t *bmx280) {
    ...
    if (err == ESP_OK) {
        if (
            ...
            ESP_LOGE("bmx280", "Sensor model may be incorrect. Please check the sensor model configuration. If unsure, "
                               "set it to AUTO.");
    ...
    ESP_LOGW("bmx280", "Probe failure: address=%hhx, id=%hhx, reason=%s", bmx280->slave, bmx280->chip_id,
             esp_err_to_name(err));
#else
    ESP_LOGW("bmx280", "Probe failure: address=%hhx, id=%hhx, reason=%s", bmx280->dev_cfg.device_address,
             bmx280->chip_id, esp_err_to_name(err));
#endif
    return err;
}
