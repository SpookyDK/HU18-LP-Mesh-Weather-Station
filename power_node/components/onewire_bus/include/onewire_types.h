...
typedef struct {
  ...
  struct onewire_bus_config_flags {
    uint32_t en_pull_up
        : 1; /*!< Set true to enable internal pull-up resistor.
                  Please note the internal pull-up resistor cannot provide
                enough current for some devices, so external pull-up resistor is
                still recommended. */
