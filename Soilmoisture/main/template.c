...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle, MOISTURE_ADC_CHANNEL, &(adc_oneshot_chan_cfg_t){
    ...
    if (adc_cali_create_scheme_curve_fitting(&cali_cfg, &cali_handle) == ESP_OK) {
        ...
        ESP_LOGW(TAG, "ADC calibration failed, voltage readings will be wrong");
    }
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Raw: %4d | Voltage: %4d mV | Moisture: %3d%%",
