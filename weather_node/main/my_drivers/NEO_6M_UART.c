...
static void evaluate_frame(ubx_frame_t *frame) {
    ...
    for (int f = 0; f < frame->frame_count; f++) {
        ...
        case 0x0201: // NAV-POSLLH
            ...
            ESP_LOGD("GPS", "Latitude:Longitude:  %.7f, %.7f", gps_shared_latitude / 1e7f, gps_shared_longitude / 1e7);
            break;
...
static void gps_init_uart() {
    ...
    response_queue = xQueueCreate(3, sizeof(ubx_frame_t *));
    nav_queue = xQueueCreate(5, sizeof(ubx_frame_t *));
