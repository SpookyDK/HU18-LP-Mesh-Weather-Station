...
typedef struct {
  uint32_t max_rx_bytes; /*!< Set the largest possible single receive size,
                              which determins the size of the internal buffer
                            that used to save the receiving RMT symbols */
 ...
 *      - ESP_ERR_INVALID_ARG: create 1-Wire bus handle failed because of
 * invalid argument
 *      - ESP_ERR_NO_MEM: create 1-Wire bus handle failed because of out of
 * memory
