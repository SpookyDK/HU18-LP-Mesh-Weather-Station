...
#include "esp_err.h"
#include "onewire_device.h" // It claims it isnt needed, yet wont compile without it
#include <stdint.h>
...
struct onewire_bus_t {
   ...
   *      - ESP_ERR_INVALID_ARG: Write bytes to 1-Wire bus failed because of
   * invalid argument
   *      - ESP_FAIL: Write bytes to 1-Wire bus failed because of other errors
   */
  esp_err_t (*write_bytes)(onewire_bus_t *bus, const uint8_t *tx_data,
   ...
   *      - ESP_ERR_INVALID_ARG: Read bytes from 1-Wire bus failed because of
   * invalid argument
   *      - ESP_FAIL: Read bytes from 1-Wire bus failed because of other errors
   */
  esp_err_t (*read_bytes)(onewire_bus_t *bus, uint8_t *rx_buf,
                          size_t rx_buf_size);
  ...
  /**
   * @brief Send reset pulse to the bus, and check if there are devices attached
   * to the bus
   *
   * @param[in] bus 1-Wire bus handle
   *
   ...
   *      - ESP_ERR_NOT_FOUND: Reset 1-Wire bus successfully but no device found
   * on the bus
