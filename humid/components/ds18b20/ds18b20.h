 ...
 *      - ESP_ERR_INVALID_ARG: Create DS18B20 device failed due to invalid
 * argument
 *      - ESP_ERR_NO_MEM: Create DS18B20 device failed due to out of memory
 *      - ESP_ERR_NOT_SUPPORTED: Create DS18B20 device failed because the device
 * is unknown (e.g. a wrong family ID code)
 ...
 *
 * @note This function assumes that the device is a DS18B20 device and there is
 ...
 *      - ESP_ERR_INVALID_ARG: Delete DS18B20 device failed due to invalid
 * argument
 ...
 *
 * @note After send the trigger command, the DS18B20 will start temperature
 * conversion. This function will delay for some while, to ensure the
 * temperature conversion won't be interrupted.
 *
 * @param[in] ds18b20 DS18B20 device handle
 * @return
 *      - ESP_OK: Trigger temperature conversion successfully
 *      - ESP_ERR_INVALID_ARG: Trigger temperature conversion failed due to
 * invalid argument
 ...
 *
 * @note After send the trigger command, all the DS18B20 devices will start
 * temperature conversion. This function will delay for some while, to ensure
 * the temperature conversion won't be interrupted.
 *
 * @param[in] bus 1-Wire bus handle
 * @return
 *      - ESP_OK: Trigger temperature conversion successfully
