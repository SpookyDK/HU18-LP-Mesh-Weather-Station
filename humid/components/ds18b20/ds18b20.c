...
typedef struct ds18b20_device_t {
    ...
    onewire_device_address_t
        addr; // if the addr is 0, we will send "ONEWIRE_CMD_SKIP_ROM" command
    uint8_t th_user1;
    uint8_t tl_user2;
...
esp_err_t
ds18b20_new_device_from_enumeration(onewire_device_t *device,
                                    const ds18b20_config_t *config,
                                    ds18b20_device_handle_t *ret_ds18b20) {
    ds18b20_device_t *ds18b20 = NULL;
    ESP_RETURN_ON_FALSE(device && config && ret_ds18b20, ESP_ERR_INVALID_ARG,
    ...
    ds18b20->resolution =
        DS18B20_RESOLUTION_12B; // DS18B20 default resolution is 12 bits

    *ret_ds18b20 = ds18b20;
    return ESP_OK;
}

esp_err_t ds18b20_new_device_from_bus(onewire_bus_handle_t bus,
                                      const ds18b20_config_t *config,
                                      ds18b20_device_handle_t *ret_ds18b20) {
    ds18b20_device_t *ds18b20 = NULL;
    ESP_RETURN_ON_FALSE(bus && config && ret_ds18b20, ESP_ERR_INVALID_ARG, TAG,
                        "invalid argument");
...
static esp_err_t ds18b20_send_command(ds18b20_device_handle_t ds18b20,
                                      uint8_t cmd) {
    // No address mode (single device connected to the bus)
    // use "Skip ROM" command
    if (ds18b20->addr == 0) {
        uint8_t tx_buffer[2] = {ONEWIRE_CMD_SKIP_ROM, cmd};
        return onewire_bus_write_bytes(ds18b20->bus, tx_buffer,
                                       sizeof(tx_buffer));
    }
...
esp_err_t ds18b20_set_resolution(ds18b20_device_handle_t ds18b20,
                                 ds18b20_resolution_t resolution) {
    ESP_RETURN_ON_FALSE(ds18b20, ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    // reset bus and check if the ds18b20 is present
    ESP_RETURN_ON_ERROR(onewire_bus_reset(ds18b20->bus), TAG,
                        "reset bus error");

    // send command: DS18B20_CMD_WRITE_SCRATCHPAD
    ESP_RETURN_ON_ERROR(
        ds18b20_send_command(ds18b20, DS18B20_CMD_WRITE_SCRATCHPAD), TAG,
    ...
    ESP_RETURN_ON_ERROR(
        onewire_bus_write_bytes(ds18b20->bus, tx_buffer, sizeof(tx_buffer)),
        TAG, "send new resolution failed");
...
esp_err_t
ds18b20_trigger_temperature_conversion(ds18b20_device_handle_t ds18b20) {
    ESP_RETURN_ON_FALSE(ds18b20, ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    // reset bus and check if the ds18b20 is present
    ESP_RETURN_ON_ERROR(onewire_bus_reset(ds18b20->bus), TAG,
                        "reset bus error");
    ...
    ESP_RETURN_ON_ERROR(ds18b20_send_command(ds18b20, DS18B20_CMD_CONVERT_TEMP),
                        TAG, "send DS18B20_CMD_CONVERT_TEMP failed");
...
esp_err_t
ds18b20_trigger_temperature_conversion_for_all(onewire_bus_handle_t bus) {
    ...
    ESP_RETURN_ON_ERROR(
        onewire_bus_write_bytes(bus, tx_buffer, sizeof(tx_buffer)), TAG,
...
esp_err_t ds18b20_get_temperature(ds18b20_device_handle_t ds18b20,
    ...
    ESP_RETURN_ON_ERROR(
        ds18b20_send_command(ds18b20, DS18B20_CMD_READ_SCRATCHPAD), TAG,
        "send DS18B20_CMD_READ_SCRATCHPAD failed");

    // read scratchpad data
    ds18b20_scratchpad_t scratchpad;
    ESP_RETURN_ON_ERROR(onewire_bus_read_bytes(ds18b20->bus,
                                               (uint8_t *)&scratchpad,
                                               sizeof(scratchpad)),
                        TAG, "error while reading scratchpad data");
    // check crc
    ESP_RETURN_ON_FALSE(onewire_crc8(0, (uint8_t *)&scratchpad, 8) ==
                            scratchpad.crc_value,
                        ESP_ERR_INVALID_CRC, TAG, "scratchpad crc error");

    const uint8_t lsb_mask[4] = {0x07, 0x03, 0x01,
                                 0x00}; // mask bits not used in low resolution
    uint8_t lsb_masked =
        scratchpad.temp_lsb & (~lsb_mask[scratchpad.configuration >> 5]);
...
esp_err_t ds18b20_get_device_address(ds18b20_device_handle_t ds18b20,
    ...
    ESP_RETURN_ON_FALSE(ds18b20 && ret_address, ESP_ERR_INVALID_ARG, TAG,
                        "invalid argument");
