...
dmi = pd.read_csv('dmi_humid.csv', sep=',', decimal='.')  # comma as separator, comma as decimal
...
plt.title('Measured Air Humidity Compared Compared To DMI', fontsize = 25)
plt.plot(our['TIME'], our['AIR HUM'], label='Measured Humidity', color='orange', linewidth=4)
plt.plot(dmi['DateTime'], dmi['Luftfugtighed'], label='DMI Hummidity', color='green', linewidth = 4)
plt.axhline()
plt.ylim(0, 100)
...
plt.subplots_adjust(left=0.07, right=0.98, top=0.95, bottom=0.15, hspace=0.3)
plt.legend(
...
plt.grid(visible=True, axis='x', linestyle='--', alpha=0.7) #makes grid
plt.show()
